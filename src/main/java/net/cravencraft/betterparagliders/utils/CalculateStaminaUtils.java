...
public class CalculateStaminaUtils {
    ...
    public static double getModifiedStateChange(PlayerMovement playerMovement) {
        ...
        double modified = switch (state) {
            case "idle" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.IDLE_STAMINA_REGEN.getDelegate());
            case "running" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.SPRINTING_STAMINA_REDUCTION.getDelegate());
            case "swimming" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.SWIMMING_STAMINA_REDUCTION.getDelegate());
            case "underwater" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.SUBMERGED_STAMINA_REGEN.getDelegate());
            case "breathing_underwater" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.WATER_BREATHING_STAMINA_REGEN.getDelegate());

            case "fast_running" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.FAST_RUNNING_STAMINA_REDUCTION.getDelegate());
            case "fast_swimming" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.FAST_SWIMMING_STAMINA_REDUCTION.getDelegate());
            case "horizontal_wall_run" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.HORIZONTAL_WALL_RUN_STAMINA_REDUCTION.getDelegate());
            case "cling_to_cliff" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.CLING_TO_CLIFF_STAMINA_REDUCTION.getDelegate());
            case "dodge" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.DODGE_STAMINA_REDUCTION.getDelegate());
            case "roll" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.ROLL_STAMINA_REDUCTION.getDelegate());
            case "climb_up" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.CLIMB_UP_STAMINA_REDUCTION.getDelegate());
            case "breakfall" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.BREAKFALL_STAMINA_REDUCTION.getDelegate());
            case "vault" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.VAULT_STAMINA_REDUCTION.getDelegate());
            case "vertical_wall_run" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.VERTICAL_WALL_RUN_STAMINA_REDUCTION.getDelegate());
            case "cat_leap" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.CAT_LEAP_STAMINA_REDUCTION.getDelegate());
            case "charge_jump" ->
                    original + player.getAttributeValue(BetterParaglidersAttributes.CHARGE_JUMP_STAMINA_REDUCTION.getDelegate());

            default -> original;
        };

        if (original > 0.0D) {
            return Math.max(modified, 0.0D);
        } else if (original < 0.0D) {
            return Math.min(-0.0001D, modified);
        } else {
            return modified;
        }
    }
