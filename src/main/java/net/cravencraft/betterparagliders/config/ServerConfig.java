...
public class ServerConfig {
    ...
    public ServerConfig(ModConfigSpec.Builder server) {
        ...
        MELEE_STAMINA_CONSUMPTION = server
                .comment("How much more/less stamina is consumed from melee attacks (multiplied onto the base amount).")
                .defineInRange("melee_stamina_consumption", 1.0, 0.0, 10.0);

        TWO_HANDED_STAMINA_CONSUMPTION = server
                .comment("How much more/less stamina is consumed from two handed attacks (multiplied onto the base amount) (stacks with MELEE_STAMINA_CONSUMPTION).")
                .defineInRange("two_handed_stamina_consumption", 2.0, 0.0, 10.0);

        ONE_HANDED_STAMINA_CONSUMPTION = server
                .comment("How much more/less stamina is consumed from one handed (and dual wielding) attacks (multiplied onto the base amount) (stacks with MELEE_STAMINA_CONSUMPTION).")
                .defineInRange("one_handed_stamina_consumption", 2.5, 0.0, 10.0);

        RANGE_STAMINA_CONSUMPTION = server
                .comment("The base amount of stamina a range attack will cost (bows added to a datapack will be added to this value).")
                .defineInRange("ranged_stamina_consumption", 5.0, 0.0, 10.0);

        BLOCK_STAMINA_CONSUMPTION = server
                .comment("The base amount of stamina blocking with a shield will cost (shields added to a datapack will be added to this value).")
                .defineInRange("block_stamina_consumption", 10.0, 0.0, 100.0);

        BLOCK_PROJECTILE_STAMINA_CONSUMPTION = server
                .comment("The base amount of stamina blocking a projectile will cost (in addition to blocking cost).")
